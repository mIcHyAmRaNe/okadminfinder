...
class okadminfinder:
    ...
    def credit(self):
        ...
        | |___| | |  \ \/ ___ ( (_| | | | | | | | || |  | | | | ( (_| | ____| |
         \_____/|_|   \_\_____|\____|_|_|_|_|_| |_||_|  |_|_| |_|\____|_____|_|
            💚 version {meta.__version__} created by {meta.__creator__} & coded by {meta.__maintainer__} 💚
        ...
        print(green, DIM, f"\tTime needed : {(time.time() - t0):.2f}", RESET_ALL)  # noqa: E501

    def get_agents(self):
        agents_path = "okadminfinder/LinkFile/user-agent.txt"
    ...
    def get_links():
        links_path = "okadminfinder/LinkFile/adminpanellinks.txt"
        links = []
    ...
    def get_proxy(prox):
        got_proxies = {"http": f"http://{prox}", "https": f"http://{prox}"}
        return got_proxies

    def proxy(self, prox, headers):
        try:
            print(okadminfinder.get_proxy(prox))
            proxies = okadminfinder.get_proxy(prox)
            with httpx.Client(headers=headers, proxies=proxies) as client:
                client.get("https://httpbin.org/get")
        except (httpx.NetworkError, httpx.ProxyError):
            print("\n\t┎───[", red, BOLD, "Proxy/Network Error", RESET_ALL, "]")  # noqa: E501
            print(
                "\t┠───╸",
                magenta,
                BOLD,
                "Check the proxy format | Reminder: 127.0.0.1:8080",
                RESET_ALL,
            )
            print("\t┠───╸", magenta, BOLD, "Check the proxy quality", RESET_ALL)  # noqa: E501
    ...
    async def url(self, website, headers, proxies):
        try:
            if okadminfinder.check_url(website, headers, proxies):
                print(
                    magenta, BOLD, website, RESET_ALL, green, "is stable\n", RESET_ALL  # noqa: E501
                )
            else:
            ...
            async with httpx.AsyncClient(headers=headers, proxies=proxies) as client:  # noqa: E501
                for url in urls:
                    pbar.update()
                    try:
